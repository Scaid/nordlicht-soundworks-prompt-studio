...
function tags(s=snap()){const p=[];if(s.modules.instrument.ready)p.push(s.modules.instrument.data.sectionTags);if(s.modules.vocal.ready)p.push(s.modules.vocal.data.dna);const engine=window.NSWMetaTagStackEngine;return p.filter(Boolean).reduce((out,document)=>engine?engine.mergeStackDocument(out,document,{repeatLast:true}).text:[out,document].filter(Boolean).join('\n\n'),'')}
...
function appendLyrics(v){const e=$('lyricsEditor'),engine=window.NSWMetaTagStackEngine;if(!e||!v)return false;e.value=engine?engine.mergeStackDocument(e.value,v,{repeatLast:true}).text:(e.value.trim()?e.value.trim()+'\n\n'+v:v);e.dispatchEvent(new Event('input',{bubbles:true}));return true}
